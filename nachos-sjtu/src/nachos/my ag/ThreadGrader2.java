...
public class ThreadGrader2 extends BasicTestGrader {
	...
	public void run() {
		...
		forkNewThread(new b());
		while (count != total) {
			System.out.println("1:"+Machine.timer().getTime());
		...
		forkNewThread(new b());
		while (count != total) {
			System.out.println("2:"+Machine.timer().getTime());
			assertTrue(Machine.timer().getTime() < 2500,
		...
		while (count != total) {
			System.out.println("3:"+Machine.timer().getTime()+"count = "+count+"; total = "+total);
