...
//		System.out.println("update "+page+"; phyPages["+entry.ppn+"]="+phyPages[entry.ppn]);
	...
	public static TranslationEntry getIPTEntry(int pid, int vpn) {
		...
		return IPT.get(new VMPage(pid, vpn));
//		if (entry == null)
...
//	}
	...
	//swap in the page if it is in swapFile; o.w. return null;
	static TranslationEntry swapIn(int ppn, int pid, int vpn) {
		Lib.assertTrue(IPTLock.isHeldByCurrentThread());
		++swapTimes;
		
		VMPage page = new VMPage(pid, vpn);
		SwapPage swapPage = swapTable.get(page);
		if (swapPage == null) return null;
		//ERROR: DON't NEED TO CHANGE FREESWAPPAGE
		
		Processor p = Machine.processor();
		TranslationEntry entry = new TranslationEntry(page.vpn, ppn, true,swapPage.readOnly,true,false);
		addIPTPage(pid, entry);
...
//		TLBLock.release();
	...
	// swap out if need
	static int obtainFreePPN(int pid) {
		Lib.assertTrue(IPTLock.isHeldByCurrentThread());
		int ppn = -1;
		if (IPT.size() < pageNum) {
...
//			TLBLock.release();
			...
			//randswap
			/*ppn = Lib.random(pageNum);
			VMPage page = phyPages[ppn];
...
//			TLBLock.release();
	...
	static void addIPTPage(int pid, TranslationEntry entry) {
		Lib.assertTrue(IPTLock.isHeldByCurrentThread());
		VMPage page = new VMPage(pid, entry.vpn);
		int ppn = entry.ppn;
